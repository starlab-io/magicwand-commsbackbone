...
var svg = d3.select("#goloris-connections-svg").append("svg")
...
d3.json("/data/goloris/" + window.application.query.test + ".json", function(error, data) {
  ...
  data.aggregate.forEach(function(d) {
  ...
  console.log("mapping ["+ name + "]");
    var blob = {
      name: name,
      values: data.aggregate.map(function(d) {
  ...
  x.domain(d3.extent(data.aggregate, function(d) { return d.date; }));
